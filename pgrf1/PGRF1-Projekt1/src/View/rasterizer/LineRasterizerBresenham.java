...
public class LineRasterizerBresenham extends LineRasterizer {
    ...
    protected void drawLine(int x1, int y1, int x2, int y2, int thickness) {
        ...
        if (Math.abs(x2 - x1) > Math.abs(y2 - y1)) {
            drawLineH(x1, y1, x2, y2, thickness);
        } else {
            drawLineV(x1, y1, x2, y2, thickness);
        }
    }

    // Implementace bresenhamova algoritmu pro vykreslování horizontálních čar
    private void drawLineH(int x1, int y1, int x2, int y2, int thickness) {
            ...
            for (var i = x1; i <= x2; i++) {
                int offset = 0;
                    ...
                    for (var j = 0; j < thickness; j++) {
                        if (y + offset > 0) {
                            img.setRGB(i, y + yoffset, super.color.getRGB());
                        }
    ...
    private void drawLineV(int x1, int y1, int x2, int y2, int thickness) {
        var img = ((RasterBufferedImage) raster).getImage();
        ...
        if (dy != 0) {
            ...
            for (var i = y1; i <= y2; i++) {
                ...
                for (var j = 0; j < thickness; j++) {
                    if (x + xoffset > 0) {
                        img.setRGB(x + xoffset, i, super.color.getRGB());
                    }
